...
class ExecutionLedgerRow(Base):
    """Durable execution ledger entry — observability only; never authorizes."""

    __tablename__ = "execution_ledger"

    id = Column(GUID(), primary_key=True)
    workflow_id = Column(GUID(), nullable=True, index=True)
    step_id = Column(GUID(), nullable=True, index=True)
    case_id = Column(GUID(), nullable=True, index=True)
    capability = Column(String(128), default="")
    ...
    observation_id = Column(GUID(), nullable=True)
    evidence_refs = Column(JSON, default=list)
    failure_reason = Column(Text, default="")
    recovery_state = Column(String(64), default="")
    actor = Column(String(128), default="system")
